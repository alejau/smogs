smogs <- function(X, y, tasks, groups, lambda=NULL, nlambda=20, model="linear", standardize=T,delta=1,conv.eps=1e-3, eps=1e-6,maxiter=100,maxiter.shotgun=300) {
  ...
  if(is.null(lambda)){
    lambda <- calcLambda(X,y,tasks,groups,nlambda,model)
  }
  ...
  for(i in 1:nlambda){
    ...
    temp.fit <- .Call("smogs", X, y, nk, groups, lambda[i], corr.factor,  model.num, conv.eps, eps, maxiter, maxiter.shotgun, PACKAGE = "smogs")
...
sogl<- function(X, y, groups, lambda=NULL, nlambda=20, model="linear", standardize=T,conv.eps=1e-3, eps=1e-6,maxiter=100,maxiter.shotgun=300) {
  ...
  fit <- NULL; nlambda <- length(lambda)
   for(i in 1:nlambda){
     temp.fit <- .Call("sogl", X, y, n, groups, lambda[i], model.num, conv.eps, eps, maxiter, maxiter.shotgun, PACKAGE = "smogs")
     if(temp.fit$converged){
       fit$converged <- c(fit$converged, temp.fit$converged) 
       fit$beta <- cbind(fit$beta, as.numeric(temp.fit$beta))
       fit$d <- cbind(fit$d, temp.fit$d)
...
calcLambda <- function(X,y,tasks,groups, nlambda,model){
  lams <- c()
  ...
  for(k in 1:K){
    ...
    Xtilde<-X[tasks==task,, drop=F] %*% diag(apply(groups %*% diag(L),1,sum))
    lams <- c(lams,abs(drop(crossprod(Xtilde[,,drop=F], resids))))
  }
  lams <- lams[lams!=max(lams)] #remove largest value
  lambda.max <- max(lams)
  if(model=="linear")
